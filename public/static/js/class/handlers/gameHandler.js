class GameHandler {
  ...
  nextFrame() {
    ...
    this.physicsHandler.nextFrame({types:["isDisappearBlock", "isEndBlock", "isKillBlock"],actions:[this.physicsHandler.disappearBlock,this.nextLevel,this.playerHandler.resetPlayer]});
    this.playerHandler.updatePlayer(
      this.physicsHandler.getPlayerBody().position,
      this.hasCollided(),
      {
        Xspeed: this.physicsHandler.getPlayerBody().velocity.x,
        Yspeed: this.physicsHandler.getPlayerBody().velocity.y,
      }
    );
	...
	//FIXME: Define where this should happen.
  ...
  levelInit() {
    const currentLevel = this.levelHandler.getLevelData(this.getCurrentLevel());
    const physicsHandler = new PhysicsHandler({
      physics: currentLevel.physics,
	  //FIXME: Items added on initialization
    }); //FIXME physics is beng added in a wierd way fix it
//Goal: get all the entity information into a m.b.rect format

	const items = []
items.push(currentLevel.player,currentLevel.obstacle,currentLevel.enemies)
	items.forEach(item=>{this.physicsHandler.addItem(item)})
